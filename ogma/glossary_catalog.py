...
CATALOG_SCHEMA = "ogma.glossary.catalog.v1"
PACK_SCHEMA = "ogma.rules.export.v1"
INSTALL_STATE_SCHEMA = "ogma.glossary.install-state.v1"
...
REPOSITORY_URL = (
    f"https://github.com/{GITHUB_OWNER}/{GITHUB_REPOSITORY}/tree/main/materials/glossaries"
)
MAX_CATALOG_BYTES = 512 * 1024
MAX_PACK_BYTES = 16 * 1024 * 1024
MAX_PACKS = 50
MAX_SELECTED_PACKS = 12
CACHE_SECONDS = 5 * 60
...
def _clean_text(value, *, field: str, maximum: int, required: bool = True) -> str:
    ...
    if (required and not text) or len(text) > maximum or any(ord(character) < 32 for character in text):
        raise ValueError(f"Glossary catalog field {field} is invalid.")
    return text


def parse_glossary_catalog(payload: object) -> list[dict]:
    if not isinstance(payload, dict) or payload.get("schema") != CATALOG_SCHEMA:
        raise ValueError("Glossary catalog schema is not supported.")
    raw_packs = payload.get("packs")
    if not isinstance(raw_packs, list) or not raw_packs or len(raw_packs) > MAX_PACKS:
        raise ValueError("Glossary catalog does not contain a valid pack list.")
    ...
    for raw_pack in raw_packs:
        if not isinstance(raw_pack, dict):
            raise ValueError("Glossary catalog pack is malformed.")
        ...
        if not PACK_ID_PATTERN.fullmatch(pack_id):
            raise ValueError("Glossary pack id is invalid.")
        if not PACK_FILENAME_PATTERN.fullmatch(filename):
            raise ValueError("Glossary pack filename is invalid.")
        if not PACK_VERSION_PATTERN.fullmatch(version):
            raise ValueError("Glossary pack version is invalid.")
        if not SHA256_PATTERN.fullmatch(digest):
            raise ValueError("Glossary pack checksum is invalid.")
        if pack_id in known_ids or filename in known_filenames:
            raise ValueError("Glossary catalog contains duplicate packs.")

        try:
            file_size = int(raw_pack.get("size", 0))
            rules_count = int(raw_pack.get("rules_count", 0))
        except (TypeError, ValueError) as exc:
            raise ValueError("Glossary pack size or rule count is invalid.") from exc
        if not 1 <= file_size <= MAX_PACK_BYTES or not 1 <= rules_count <= 100_000:
            raise ValueError("Glossary pack size or rule count is outside the allowed range.")

        raw_sources = raw_pack.get("sources", [])
        if not isinstance(raw_sources, list) or len(raw_sources) > 40:
            raise ValueError("Glossary pack sources are invalid.")
        sources = [
            _clean_text(source, field="source", maximum=80)
...
class GlossaryCatalogManager:
    def __init__(
        ...
        *,
        ...
        self.bundled_root = self.bundle_root / "materials" / "glossaries"
        self.state_path = self.data_dir / "installed-glossaries.json"
    ...
    def catalog(self, *, force: bool = False) -> dict:
        ...
        installed = self._load_install_state().get("installed", {})
        public_packs = []
        for pack in packs:
    ...
    def download_packs(self, pack_ids: object) -> list[dict]:
        ...
        unknown = [pack_id for pack_id in selected_ids if pack_id not in by_id]
        if unknown:
            raise ValidationError("Выбранного набора больше нет в каталоге. Обновите список и попробуйте снова.")
        ...
        for pack_id in selected_ids:
            ...
            if (
                ...
                or len(payload["rules"]) != entry["rules_count"]
            ):
    ...
    def record_installed(self, entries: list[dict]) -> None:
        ...
        try:
            ...
            # The glossary itself is already installed; state only decorates the catalog UI.
            return
    ...
    def _normalize_selection(self, pack_ids: object) -> list[str]:
        if not isinstance(pack_ids, list) or not pack_ids or len(pack_ids) > MAX_SELECTED_PACKS:
            raise ValidationError("Выберите от одного до двенадцати наборов глоссария.")
        selected: list[str] = []
        for raw_pack_id in pack_ids:
            pack_id = str(raw_pack_id or "").strip()
            if not PACK_ID_PATTERN.fullmatch(pack_id):
                raise ValidationError("Выбран некорректный набор глоссария.")
            if pack_id not in selected:
                selected.append(pack_id)
        if not selected:
            raise ValidationError("Выберите хотя бы один набор глоссария.")
        return selected
    ...
    def _load_catalog(self, *, force: bool) -> tuple[list[dict], str]:
        ...
        if not self.frozen and (self.bundled_root / "manifest.json").is_file():
            packs = self._read_bundled_catalog()
            source = "bundled"
        else:
            try:
                packs = self._read_remote_catalog()
                source = "github"
    ...
    def _read_remote_catalog(self) -> list[dict]:
        raw = self._fetch(CATALOG_URL, MAX_CATALOG_BYTES)
        return self._parse_catalog_bytes(raw)

    def _read_bundled_catalog(self) -> list[dict]:
        path = self.bundled_root / "manifest.json"
        try:
            raw = path.read_bytes()
        except OSError as exc:
            raise ExternalOperationError("Не удалось открыть встроенный каталог глоссариев.") from exc
        if len(raw) > MAX_CATALOG_BYTES:
            raise ExternalOperationError("Встроенный каталог глоссариев слишком большой.")
        return self._parse_catalog_bytes(raw)
    ...
    def _parse_catalog_bytes(self, raw: bytes) -> list[dict]:
        try:
            payload = json.loads(raw.decode("utf-8-sig"))
            if not json_within_limits(payload):
                raise ValueError("Glossary catalog exceeds JSON limits.")
            return parse_glossary_catalog(payload)
        except (UnicodeError, json.JSONDecodeError, ValueError) as exc:
            raise ExternalOperationError("Каталог глоссариев повреждён или использует неподдерживаемый формат.") from exc

    def _load_pack_bytes(self, entry: dict, source: str) -> bytes:
        if source == "bundled":
            path = self.bundled_root / entry["filename"]
            try:
                raw = path.read_bytes()
            except OSError as exc:
    ...
    def _fetch(self, url: str, maximum_bytes: int) -> bytes:
        ...
        try:
            ...
            if exc.code == 404:
                message = "Каталог глоссариев ещё не опубликован в GitHub."
            elif exc.code in {403, 429}:
                message = "GitHub временно ограничил загрузку каталога. Попробуйте позже."
            else:
                message = "GitHub не смог отдать каталог глоссариев."
            raise ExternalOperationError(message) from exc
        except (URLError, TimeoutError, ExternalHttpRejected, OSError) as exc:
            raise ExternalOperationError("Не удалось связаться с GitHub для загрузки глоссария.") from exc
    ...
    def _load_install_state(self) -> dict:
        ...
        if not isinstance(payload, dict) or payload.get("schema") != INSTALL_STATE_SCHEMA:
            return {"schema": INSTALL_STATE_SCHEMA, "installed": {}}
        return payload
